...
import { getRegionProvince, getRegionCity, getRegionCounty, updateAddress } from '@/api/address'
...
export default {
  ...
  async created () {
    // 再次调用address/getAddressListAcion ，确保数据加载完成后在渲染
  ...
  computed: {
    ...
    selecteAddress () {
      return this.addressList.filter(item => item.user_address_id === this.addressId)
    }
  },
  methods: {
    async goCascader () {
    ...
    async subimtHandle () {
      ...
      const { status } = await updateAddress(this.addressId, this.formData)
      if (status === 0) {
    ...
    change () {
      this.selecteAddress.forEach(item => {
        this.formData.name = item.delivery_name
        this.formData.tel = item.delivery_phone
        this.formData.fieldValue = item.delivery_address
        this.formData.address = item.detail_address
      })
    }
  },
  destroyed () {
